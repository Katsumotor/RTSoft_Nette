    ...
    public function createComponentProjectForm() {
        ...
        $users = $form->addDynamic('users', function (Container $user) {
            $user->addSelect('name', 'Zadejte účastníka na projektu', $this->queryProjects->getAllPerson())
                    ->setPrompt("")
                    ->setRequired(false)
                    ->addRule(\App\Model\Validator::ISALREADYSELECTPERSON, 'Tento pracovník je jíž v seznamu', array($this['projectForm']['users']->getValues(), $user->values));
        }, $this->queryProjects->getCountPersonInDB());
        ...
        if ($row) {
            ...
            if (!$form->isSubmitted()) {
                ...
                foreach ($row->related('persononproject') as $person) {
                    $form['users'][$i]->setValues(['name' => $person->id_person]);
                    $i++;
                }
            }
            return true;
